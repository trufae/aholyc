...
SRC = src/main.c src/lex.c src/parse.c src/util.c \
...
mhc: $(OBJ) src/embed.o
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(OBJ) src/embed.o
	-strip $@ 2>/dev/null || true

$(OBJ): src/mhc.h

# embedded runtime + prelude sources
src/embed.c: tools/file2c runtime/rt.c runtime/rt.js runtime/prelude.hc
