...
How aholyc lays out data, manages the heap, and isolates per-thread state —
...
aholyc keeps that worldview and maps it onto each target as directly as it
can.
...
* A pointer is a byte address. Pointer arithmetic scales by the element
  ...
  per step in aholyc (TempleOS adds 0; Terry's guidelines say don't use it).
