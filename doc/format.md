# `aholyc fmt` — HolyC source formatting

`aholyc fmt` formats and format-checks HolyC sources so a codebase can
...
consistent conventional style, and that is what `aholyc fmt` enforces:
...
* rows in a brace-delimited initializer remain aligned because their
  trailing comma completes the row;
...
```console
$ aholyc fmt file.HC ...        # print formatted source to stdout
$ aholyc fmt -w file.HC ...     # rewrite files in place (only if changed)
$ aholyc fmt -q file.HC ...     # check: list files needing formatting,
                             #        exit 1 if any (nothing rewritten)
$ aholyc fmt - < in > out       # filter stdin to stdout ('-' or no args)
...
| variable | meaning |
|----------|---------|
| `AHOLYC_FMT_INDENT=n` | spaces per nesting level (default 2) |
| `AHOLYC_FMT_BRACES=0` | don't move function/class `{` to its own line |
