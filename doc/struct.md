...
How mhc lays out `class` and `union` types, what `sizeof` and `offset`
...
foreign struct. mhc itself uses it in `runtime/exe.hc` to pin the HolyC
...
the TempleOS instruction pointer. mhc compiles it to a tiny runtime call
...
instruction's own address); mhc has no inline assembler, so that meaning
does not apply.
