/* ahc LLVM-IR backend: emits textual .ll and builds a native executable
 * with the external LLVM toolchain (clang, or llc + system cc). ahc never
...
static char *objref(Obj *v) {
	...
	if (v->is_func) {
		if (cur_prog && v == cur_prog->startup) {
			return xstrdup (ahc_obj_mode? "@__hc_ctor": "@__hc_start");
		}
...
static void emit_func(Obj *fn) {
	...
	bool exported = fn->is_public || (is_start && !ahc_obj_mode);
...
static void ll_emit(Program *prog, FILE *out) {
	o = out;
	cur_prog = prog;
	fprintf (o, "; generated by ahc (HolyC -> LLVM IR)\n\n");
	/* strings */
...
static int ll_build(const char *artifact, const char *outpath,
	...
	if (have_cmd ("clang")) {
		...
		argv[i++] = rtpath;
		for (int k = 0; k < ahc_nccflags; k++) {
			argv[i++] = ahc_ccflags[k];
		}
		argv[i++] = "-lm";
		argv[i] = NULL;
