...
static char *objref(LlGen *lg, Obj *v) {
	if (v->is_extern || v->is_public) {
		if (v->is_extern && v->link_name) {
			/* LLVM's \01 prefix suppresses the object format's automatic
			 * user-label prefix, matching an explicit assembler label exactly. */
			return xasprintf (lg->cc, "@\"\\01%s\"", v->link_name);
		}
...
static char *asm_objref(void *ctx, Obj *v) {
	return objref ((LlGen *)ctx, v);
}
...
static void emit_stmt(LlGen *lg, Node *n) {
	...
	case ND_ASM:
		ensure_block (lg);
		asm_emit_llvm_inline (lg->cc, lg->out, n->asm_block,
			lg, asm_objref);
		break;
	case ND_TRY: {
