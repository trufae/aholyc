/* mhc LLVM-IR backend: emits textual .ll and builds a native executable
 * with the external LLVM toolchain (clang, or llc + system cc). mhc never
...
static char *objref(Obj *v) {
	...
	if (v->is_func) {
		if (cur_prog && v == cur_prog->startup) {
			return xstrdup (mhc_obj_mode? "@__hc_ctor": "@__hc_start");
		}
...
static void emit_func(Obj *fn) {
	...
	bool exported = fn->is_public || (is_start && !mhc_obj_mode);
...
static void ll_emit(Program *prog, FILE *out) {
	o = out;
	cur_prog = prog;
	fprintf (o, "; generated by mhc (HolyC -> LLVM IR)\n\n");
	/* strings */
