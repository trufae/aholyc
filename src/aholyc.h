...
typedef struct AholyAsm AholyAsm;
typedef struct AholyAsmOperand AholyAsmOperand;
...
typedef enum {
	...
	ND_ASM,            /* target assembly statement (asm_block) */
	ND_NOP,
} NodeKind;
...
struct Obj {
	...
	char *link_name;    /* explicit `_extern ASM_NAME Type HolyCName` alias */
...
struct AholyAsmOperand {
	AholyAsmOperand *next;
	Token *tok;
	char *name;
	Obj *var;
	int marker, index;
...
struct AholyAsm {
	AholyAsm *next;     /* Program.asms: file-scope blocks only */
	Token *tok;         /* `asm` token, for backend diagnostics */
	char *text;         /* normalized target assembler source */
	AholyAsmOperand *operands; /* function-scope &name address candidates */
	AholyAsmOperand *operand_tail;
	int nmarkers, noperands;
	int id;             /* makes TempleOS @@ labels block-local */
};
...
typedef struct {
	...
	StrLit *strings;
	AholyAsm *asms;    /* file-scope asm blocks, never startup statements */
	Token *asm_tok;    /* first asm block, for unsupported-backend errors */
	int nstrings;
} Program;
...
struct Aholyc {
	...
	int exe_serial;
	bool verbose, keep, shared, use_hints, use_asm, use_pic;
	bool use_stack_protector, error_active;
};
...
typedef char *(*AsmObjNameFn)(void *ctx, Obj *var);
