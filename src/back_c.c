...
static void emit_bits_begin(CGen *cg, Type *ty) {
	if (signed_i1 (ty)) {
		...
		sb_printf (cg->out, "(hc_i64)(%s _BitInt(%d))",
...
static void emit_binary(CGen *cg, Node *n) {
	sb_printf (cg->out, "(");
	emit_val (cg, n->lhs);
	sb_printf (cg->out, " %s ", binop (n->kind));
	emit_val (cg, n->rhs);
	sb_printf (cg->out, ")");
}

static void emit_unsigned_binary(CGen *cg, Node *n) {
	sb_printf (cg->out, "(hc_i64)((hc_u64)");
	emit_val (cg, n->lhs);
	sb_printf (cg->out, " %s (hc_u64)", binop (n->kind));
	emit_val (cg, n->rhs);
	sb_printf (cg->out, ")");
}
...
static void emit_val(CGen *cg, Node *n) {
	switch (n->kind) {
	case ND_NUM:
		sb_printf (cg->out, "(hc_i64)0x%llxULL", (unsigned long long)n->ival);
		break;
	case ND_FNUM:
		emit_fnum (cg, n->fval);
		break;
	...
	case ND_VAR: {
		...
		if (is_fs_obj (v)) {
			...
			sb_printf (cg->out, "(hc_i64)%s", objname (cg, v));
		} else {
			sb_printf (cg->out, "%s", objname (cg, v));
		}
		break;
	...
	case ND_ASSIGN: {
		...
		if (l->kind == ND_VAR && !is_agg (l->ty)) {
			sb_printf (cg->out, "(%s = ", objname (cg, l->var));
			if (l->ty->bits) {
	...
	case ND_CAST: {
		...
		if (n->bit_cast && to->kind == TY_F64) {
			...
			if (to->kind == TY_INT && to->size < 8) {
				sb_printf (cg->out, "(hc_i64)(%s)(hc_i64)", scalar_ctype (to));
			} else {
				sb_printf (cg->out, "(hc_i64)");
			}
			emit_val (cg, n->lhs);
	...
	case ND_SUB: {
		...
		if (lp && rp && n->kind == ND_SUB) {
			...
			sb_printf (cg->out, " - ");
			emit_val (cg, n->rhs);
			sb_printf (cg->out, ") / %d)", elem_size (lt));
			break;
		}
		if (lp) {
			sb_printf (cg->out, "(");
	...
	case ND_NOP:
		...
		break;
	default:
		error (cg->cc, "C backend: unexpected node kind %d in expression", n->kind);
	}
}
...
static void emit_addr(CGen *cg, Node *n) {
	...
	case ND_MEMBER:
		if (n->member_ref->offset) {
			sb_printf (cg->out, "(");
			emit_addr (cg, n->lhs);
			sb_printf (cg->out, " + %d)", n->member_ref->offset);
		} else {
			emit_addr (cg, n->lhs);
		}
		break;
	default:
		error (cg->cc, "C backend: not an lvalue (node kind %d)", n->kind);
	}
}
...
static void emit_stmt(CGen *cg, Node *n, int ind) {
	...
	case ND_FOR:
		...
		ind_ (cg, ind);
		sb_printf (cg->out, "for (;;) {\n");
		if (n->cond) {
			ind_ (cg, ind + 1);
			sb_printf (cg->out, "if (!");
			emit_truth (cg, n->cond);
			sb_printf (cg->out, ") break;\n");
		}
...
static void emit_func(CGen *cg, Obj *fn) {
	...
	for (Obj *v = fn->locals; v; v = v->next) {
		sb_printf (cg->out, "\t");
		if (v->align) {
			sb_printf (cg->out, "_Alignas(%d) ", v->align < v->ty->align? v->ty->align: v->align);
		}
		if (is_agg (v->ty)) {
			sb_printf (cg->out, "hc_i64 %s[%d] = {0};\n", objname (cg, v),
				(v->ty->size + 7) / 8? (v->ty->size + 7) / 8: 1);
		} else if (v->ty->kind == TY_F64) {
			sb_printf (cg->out, "hc_f64 %s = 0;\n", objname (cg, v));
		} else if (v->ty->bits && !signed_i1 (v->ty) && !v->address_taken) {
			sb_printf (cg->out, "%s _BitInt(%d) %s = 0;\n",
				v->ty->is_unsigned? "unsigned": "signed", v->ty->bits,
				objname (cg, v));
		} else {
			sb_printf (cg->out, "%s %s = 0;\n", scalar_ctype (v->ty), objname (cg, v));
		}
	}
	cg->try_depth = 0;
	cg->nhandlers = 0;
...
static void c_emit(Aholyc *cc, Program *prog, StrBuf *out,
	...
	CGen *cg = &gen;
	sb_printf (cg->out, "/* generated by aholyc (HolyC -> C) */\n");
	if (object_mode) {
	...
	for (StrLit *s = prog->strings; s; s = s->next) {
		...
		for (int i = 0; i < s->len; i++) {
			sb_printf (cg->out, "%d,", (unsigned char)s->data[i]);
		}
		sb_printf (cg->out, "0};\n");
	}
	for (Obj *g = prog->globals; g; g = g->next) {
