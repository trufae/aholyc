#!/bin/sh
# ahc test harness: build + run every example on every available backend
...
for b in $backends; do
	for f in examples/*.HC; do
		...
		[ -f "$exp" ] || continue
		if ! ./ahc -b "$b" -o "tests/out/$n-$b" "$f" 2>"tests/out/$n-$b.err"; then
			echo "FAIL build $b/$n"
...
for b in $backends; do
	[ "$b" = js ] && continue
	if ./ahc -c -b "$b" tests/mod_a.HC -o "tests/out/mod_a-$b.o" 2>"tests/out/mod-$b.err" &&
	   ./ahc -c -b "$b" tests/mod_b.HC -o "tests/out/mod_b-$b.o" 2>>"tests/out/mod-$b.err" &&
	   ./ahc -b "$b" "tests/out/mod_a-$b.o" "tests/out/mod_b-$b.o" -o "tests/out/mod-$b" 2>>"tests/out/mod-$b.err"; then
...
if [ -f tests/out/libhctest.a ]; then
	for b in $backends; do
		[ "$b" = js ] && continue
		if ./ahc -b "$b" tests/uselib.HC -L tests/out -lhctest \
...
if cc -pthread -c tests/tls_threads.c -o tests/out/tls_threads.o 2>/dev/null; then
	for b in $backends; do
		[ "$b" = js ] && continue
		if ./ahc -b "$b" tests/tls_threads.HC tests/out/tls_threads.o -lpthread \
...
./ahc fmt tests/out/fmt_in.HC > tests/out/fmt_got.HC 2>/dev/null
cmp -s tests/out/fmt_exp.HC tests/out/fmt_got.HC || fmtok=0
./ahc fmt -q tests/out/fmt_in.HC >/dev/null && fmtok=0   # must exit 1
./ahc fmt -q - < tests/out/fmt_got.HC || fmtok=0         # must exit 0
for f in examples/*.HC; do
	n=$(basename "$f" .HC)
	./ahc fmt "$f" > "tests/out/fmt-$n.HC" || { fmtok=0; continue; }
	./ahc fmt - < "tests/out/fmt-$n.HC" | cmp -s - "tests/out/fmt-$n.HC" || fmtok=0
	[ -f "tests/expected/$n.out" ] || continue
	if ./ahc -b c "tests/out/fmt-$n.HC" -o "tests/out/fmt-$n" 2>/dev/null; then
