...
# stdin as a source: mhc -r < prog.HC builds a scratch ./.a.out, runs it,
# and removes it — nothing is left behind in the working directory
printf '%s\n' '"stdin ok\n";' > tests/out/stdin.HC
rm -rf tests/out/stdin.d
mkdir -p tests/out/stdin.d
for b in $backends; do
	out=$(cd tests/out/stdin.d && ../../../mhc -b "$b" -r < ../stdin.HC 2>"../stdin-$b.err")
...
./mhc -S -b c -o - - < tests/out/stdin.HC 2>/dev/null | grep -q __hc_start || stdinok=0
# empty stdin (no file args) is a usage error, not a silent empty a.out
(cd tests/out/stdin.d && ../../../mhc < /dev/null) >/dev/null 2>&1 && stdinok=0
[ -z "$(ls -A tests/out/stdin.d)" ] || stdinok=0
# -o - only means stdout with -S
echo '"x";' | ./mhc -o - >/dev/null 2>&1 && stdinok=0
if [ "$stdinok" = 1 ]; then
